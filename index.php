...
Hack version of the original page to set accountancy on Product
This version allow to sort product by catégories and to set the same accountancy for all selected Products

All modification are wrapped in a 
/** Hack start **/
//... 
/** Hack end **/
...
if ($action == 'update') {
	...
	if (!empty($btn_changeaccount)) {
		...
		if (!empty($chk_prod)) {
			...
			{
				/** hack start **/
				if( GETPOST('codeventil_all') )
					$accounting_account_id = GETPOST('codeventil_all');
				else
				/** hack end **/
...
/** Hack start **/
if(isset($_POST["categories"]) && count($_POST["categories"])) {
	
	$sql.= " LEFT JOIN ".MAIN_DB_PREFIX."categorie_product as catp ON";
	$sql.=" p.rowid = catp.fk_product";
}
/** Hack end **/
...
if(isset($_POST["categories"]) && count($_POST["categories"])) {
	...
	$sql.=" AND catp.fk_categorie IN (" . implode($_POST["categories"],",") . ")";
	$conf->global->MAIN_DISABLE_FULL_SCANLIST = true;
...
if ($result)
{
	...
	/** Hack start **/
	//> add a categorie / tag selection field
    ...
    /** hack start **/
	//>> add a accountanct field to modify all selected prodcuts
	print '<table class="noborder centpercent">';
	print '<tr><td>'.$langs->trans("UwProductAccountingAccountSelectForAll").'</td><td colspan="3">';
	$defaultvalue='';
	if ($accounting_product_mode == 'ACCOUNTANCY_BUY') {
	...
	print $form->select_account($defaultvalue, 'codeventil_all', 1, array(), 1);
	print "</td></tr>\n";
	print "</table>\n";
	/** hack end **/
